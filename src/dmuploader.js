...
(function($) {
    ...
    DmUploader.prototype.queueFiles = function(files) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            var file_type = file.name.substr(file.name.lastIndexOf(".") + 1).toLowerCase();
            if ((this.settings.allowedTypes != '*') &&
                !file_type.match(this.settings.allowedTypes)) {
        ...
        $.ajax({
            ...
            success: function(data, message, xhr) {
                if(data.status==1){
                    ...
                    widget.settings.onUploadError.call(widget.element, widget.queuePos, data.msg);
                }
            },
